...
public class SysBaseApiImpl implements ISysBaseAPI {
	...
	public List<LoginUser> getAllUser() {
		...
		List<LoginUser> res=new ArrayList<>();
...
//		}
	...
	public void updateTableStatus(String table, String statusField, String statusValue,String idField,String idValue) {
		 sysDictService.updateTableStatus(table,statusField,statusValue,idField,idValue);
	}
...
//			}
	...
	@Override
	@Transactional
	public void saveDictModel(String dictCode, String dictText, HashMap<String, String> dictItems) {
		...
		if(list!=null && list.size()>0)
		{
			for (SysDict sysDict : list) {
				sysDictService.deleteByDictId(sysDict);
			}
		}
		SysDict sysDict=new SysDict();
		sysDict.setDictCode(dictCode);
		sysDict.setDictName(dictText);
		sysDict.setType(1);
		sysDictService.save(sysDict);

		for (String itemkey : dictItems.keySet()) {
			SysDictItem sysDictItem=new SysDictItem();
			sysDictItem.setDictId(sysDict.getId());
			sysDictItem.setItemText(dictItems.get(itemkey));
			sysDictItem.setItemValue(itemkey);
