...
@Api(tags="库存期间")
@RestController
@RequestMapping("/base/mvperiod")
@Slf4j
public class MvperiodController extends JeecgController<Mvperiod, IMvperiodService> {
	@Autowired
	private IMvperiodService mvperiodService;
	...
	public Result<IPage<Mvperiod>> queryPageList(Mvperiod mvperiod,
		...
		QueryWrapper<Mvperiod> queryWrapper = QueryGenerator.initQueryWrapper(mvperiod, parameterMap);
		if(!(mvperiod.getCompanyid()!=null  && !mvperiod.getCompanyid().equalsIgnoreCase(""))){
			return  Result.OK("");
		}
		Page<Mvperiod> page = new Page<Mvperiod>(pageNo, pageSize);
		queryWrapper.orderByAsc("perioddate");
		IPage<Mvperiod> pageList = mvperiodService.page(page, queryWrapper);
	...
	//@RequiresPermissions("org.br.erp:mvperiod:add")
	@PostMapping(value = "/add")
	public Result<String> add(@RequestBody Mvperiod mvperiod) {
	...
	public Result<String> addPeriod(@RequestParam("companyid")String companyid){
		mvperiodService.addMvPerioid(companyid);
	...
	public Result<String> changePeriodState(@RequestParam("companyid")String companyid,@RequestParam("perioddate")String perioddate,
		...
		mvperiodService.changeMvPeriodState(companyid,perioddate,periodState);
	...
	public Result<String> edit(@RequestBody Mvperiod mvperiod) {
	...
	public Result<Mvperiod> queryById(@RequestParam(name="id",required=true) String id) {
		Mvperiod mvperiod = mvperiodService.getById(id);
		if(mvperiod==null) {
    ...
    */
    ...
    public ModelAndView exportXls(HttpServletRequest request, Mvperiod mvperiod) {
    ...
    */
    ...
    public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
        return super.importExcel(request, response, Mvperiod.class);
    }
