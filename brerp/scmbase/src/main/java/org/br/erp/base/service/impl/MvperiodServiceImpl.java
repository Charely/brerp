 ...
 */
@Service
public class MvperiodServiceImpl extends ServiceImpl<MvperiodMapper, Mvperiod> implements IMvperiodService {

    @Autowired
    private MvperiodMapper mvperiodMapper;

    @Override
    @Transactional
    public void addMvPerioid(String companyid) {
        String nowDate = ERPUtils.getDateString(new Date(),"yyyyMM");
        QueryWrapper<Mvperiod> queryWrapper=new QueryWrapper<>();
        queryWrapper.like("perioddate",nowDate.substring(0,4));
        queryWrapper.eq("companyid",companyid);
        List<Mvperiod> mvperiods = mvperiodMapper.selectList(queryWrapper);
        if(!(mvperiods!=null && mvperiods.size()>=12)){
            //todo 创建12行期间
            for(int i=1;i<13;i++){
                Mvperiod mvperiod=new Mvperiod();
...
//             }
            ...
            QueryWrapper<Mvperiod> nextQueryWrapper=new QueryWrapper<>();
            ...
            List<Mvperiod> nextPeriods = mvperiodMapper.selectList(nextQueryWrapper);
            ...
            if(Integer.parseInt(month) < 13){
                //将未来期间为未来
                 for(int j=Integer.parseInt(month);j<13;j++){
                     QueryWrapper<Mvperiod> futureQueryWrapper=new QueryWrapper<>();
                     ...
                     List<Mvperiod> futurePeriods= mvperiodMapper.selectList(futureQueryWrapper);
                     if(futurePeriods!=null && futurePeriods.size()>0){
                         Mvperiod futureperiod = futurePeriods.get(0);
            ...
            if(Integer.parseInt(month) >0){
                for(int k=1;k<Integer.parseInt(month)-1;k++){
                    QueryWrapper<Mvperiod> postQueryWrapper=new QueryWrapper<>();
                    ...
                    List<Mvperiod> postPeriods = mvperiodMapper.selectList(postQueryWrapper);
                    if(postPeriods!=null && postPeriods.size()>0){
                        Mvperiod postperiod = postPeriods.get(0);
    ...
    public boolean checkPeriodState(String companyid, String periodDate) {
        ...
        List<Mvperiod> mvperiods = mvperiodMapper.selectList(queryWrapper);
        if(mvperiods!=null && mvperiods.size()>0){
            Mvperiod mvperiod = mvperiods.get(0);
