...
public class DictAspect {
    ...
    @Pointcut("execution(public * org.jeecg.modules..*.*Controller.*(..)) " +
            " || execution(public * org.br.erp..*.*Controller.*(..)) " +
    ...
    private Object parseDictText(Object result) {
        if (result instanceof Result) {
            if (((Result) result).getResult() instanceof IPage) {
                ...
                String customCode=getCustomCode(records);
                ...
                for (JSONObject record : items) {
                    ...
                    //判断当前行是否存在自定义字段注解
                    if(!customCode.equalsIgnoreCase("")){
                        String objectTable ="";
                        String tableid="";
                        //获取当前自定义字段注解
                        List<CustomEntity> customEntities = baseCommonService.selectCustomCode(customCode);
                        for (CustomEntity customEntity : customEntities) {
                            objectTable = customEntity.getObjectTable();
                            tableid= record.get("id").toString();
                            String tablecode = customEntity.getCustomCode();
                            String textvalue = baseCommonService.selecttablevalue(objectTable, "id", tableid, tablecode);
                            record.put(tablecode,textvalue);
                        }
                    }
                }
                ...
                List<Object> res = ((Result<List<Object>>) result).getResult();
                List<JSONObject> items = new ArrayList<>();
                if(res!=null && res.size()>0){
                    Object o = res.get(0);
                    if(!checkHasCustomCode(res)){
                        return result;
                    }
                    String customCode = getCustomCode(res);
                    ...
                    ((Result<List>) result).setResult(Arrays.asList(items.toArray()));
                }
            }
        }
        return result;
    ...
    private Map<String, List<DictModel>> translateAllDict(Map<String, List<String>> dataListMap) {
        ...
        for (String dictCode : dataListMap.keySet()) {
          ...
          //  baseCommonService.inserttotmptable("tmptable",dataList);
    ...
    private Boolean checkHasCustomCode(List<Object> records){
        if(oConvertUtils.isNotEmpty(records) && records.size()>0){
            Object currentobject = records.get(0);
            Class<?> aClass = currentobject.getClass();
            if(oConvertUtils.isNotEmpty(aClass.getAnnotation(CustomCode.class))){
                return true;
            }
        }
        return false;
    ...
    private String getCustomCode(List<Object> records){
        if(oConvertUtils.isNotEmpty(records) && records.size()>0){
            Object clazz = records.get(0);
            if(oConvertUtils.isNotEmpty(clazz.getClass().getAnnotation(CustomCode.class))){
                CustomCode annotation = clazz.getClass().getAnnotation(CustomCode.class);
                return annotation.objectCode();
            }
        }
        return "";
    }
