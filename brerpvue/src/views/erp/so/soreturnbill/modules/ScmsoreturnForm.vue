         ...
         <a-row>
          ...
          <a-col :xs="24" :sm="6">
            <a-form-model-item label="红单" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="isred">
              <a-checkbox v-model="model.isred" placeholder="请选择红单" disabled="true" ></a-checkbox>
            </a-form-model-item>
          </a-col>
          <a-col :xs="24" :sm="6">
        ...
        :loading="scmsoitemTable.loading"
          ...
          :rowNumber="true"
           ...
           <template slot="toolbarSuffix">
            <a-button type="primary" @click="getReferSoData" :disabled="formDisabled">参照销售订单</a-button>
          </template>
          </j-vxe-table>

        </a-tab-pane>
      </a-tabs>
      <returnso-referso ref="returnsoReferso" @ok="getselectdata"></returnso-referso>
    </a-spin>
</template>
...
import ReturnsoReferso from '../../../base/custom/CReturnsoReferso.vue'
...
import { getAction } from '../../../../../api/manage'
  export default {
    ...
    watch:{
      'model.companyid':{
        handler(val,old){
          localStorage.setItem('returnso_companyid',val);
        }
      },
      'model.customerid':{
        handler(val,old){
          localStorage.setItem('returnso_customerid',val)
        }
      }
    },
    created () {
      ...
      getAction(sourl,params).then(res=>{
        if(res.success){
          ...
          for(let i=0;i<sores.length;i++){
             if(that.model.customerid === undefined || that.model.customerid === ''){
                let par={id:sores[i].parentid}
                getAction('/so/scmso/queryById',par).then(res=>{
                  if(res.success){
                    that.model.customerid=res.result.customerid
                  }
                })
             }
            const newColumnData={
              ...
              qty:(sores[i].qty - sores[i].returnqty).toFixed(2),
              uomid:sores[i].uomid,
              taxinprice:sores[i].taxinprice,
              taxrate:sores[i].taxrate,
              taxexprice:sores[i].taxexprice,
              taxexvalue:(sores[i].taxexprice * (sores[i].qty - sores[i].returnqty)).toFixed(2),
              taxinvalue:parseFloat((sores[i].qty - sores[i].returnqty)*sores[i].taxinprice).toFixed(2),
              fromitemid:sores[i].id,
              fromid:sores[i].parentid,
            }
           this.scmsoitemTable.dataSource.push(newColumnData);
          }
        }
      })
    },
    getReferSoData(){
        this.$nextTick(() => {
        this.$refs.returnsoReferso.show('其它', '销售订单', "", "1,3", this.model.customerid)
      })
      },
