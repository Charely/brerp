          ...
          <a-row :gutter="24">
            ...
            <a-col :md="6" :sm="24">
              <a-form-item label="供应商" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
                <c-select-vendor placeholder="请选择供应商进行查询" v-model="queryParam.vendorid"></c-select-vendor>
              </a-form-item>
            </a-col>
            <a-col :md="6" :sm="24">
  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          { title: '供应商名称', dataIndex: 'vendorname', width: 100 },
          ...
          {
            title:'待开票数量',dataIndex:'uninstockqty',width:100,
            customRender(text,row,index){
               return row.qty - row.invoiceqty;
            }
          },
          {
        ...
        dataSourceDetail: [],
        url: {
          list: "/inventory/scminventoy/referinstock"
        }
      }
    },
    computed: {
    ...
    methods: {
      show (type, subType, organType, status,vendorid) {
        ...
        let curcompanyid=localStorage.getItem("poinvoice_companyid");
        if(curcompanyid!=null && curcompanyid!="undefined" && curcompanyid != undefined){
          this.queryParam.companyid=curcompanyid;
        }else{
      ...
      handleOk () {
        ...
        for(let i=0; i< this.selectBillRows.length;i++){
          if(curvendorid == ''){
            curvendorid=this.selectBillRows[i].vendorid;
          }else{
            if(curvendorid != this.selectBillRows[i].vendorid){
