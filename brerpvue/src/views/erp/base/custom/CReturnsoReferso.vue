  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            customRender:(text,row,index)=>{
                let curreturnqty=row['returnqty']
               if(row['returnqty'] === null || row['returnqty'] === undefined){
                    curoutqty=0;
               }
               return row['soqty'] - curreturnqty
            }
          },
          {
            title: '订单数量', dataIndex: 'soqty', width: 100,
          },
          {
            title: '已退货数量', dataIndex: 'returnqty', width: 100,
            customRender:(text)=>{
                if(text){
        ...
        dataSourceDetail: [],
        url: {
          list: "/so/scmso/getrefersolist/1"
        }
      }
    },
    computed: {
    ...
    methods: {
      show (type, subType, organType, status,customerid) {
        ...
        this.queryParam.companyid=localStorage.getItem("returnso_companyid");
