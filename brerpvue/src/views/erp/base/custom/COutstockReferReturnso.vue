  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            customRender:(text,row,index)=>{
                let curoutqty=row['outqty']
               if(row['outqty'] === null || row['outqty'] === undefined){
                    curoutqty=0;
               }
               return (row['soqty'] - curoutqty).toFixed(2)
            }
          },
          {
          ...
          {
            title: '已出库数量', dataIndex: 'outqty', width: 100,
            customRender:(text)=>{
                if(text){
        ...
        dataSourceDetail: [],
        url: {
          list: "/so/scmso/getrefersolist/2"
        }
      }
    },
    computed: {
