  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          {
            title: '委外数量', dataIndex: 'qty', width: 100,
          },
          {
            title:'待收货数量',dataIndex:'unqty',width:100,
            customRender(text,record,index){
              if(record){
                return (record["qty"]-record["receiptreqqty"]).toFixed(2);
              }
            }
          },
          {
        ...
        dataSourceDetail: [],
        url: {
          list: "/outsource/scmoutsource/receiptreqreferwwlist"
        }
      }
    },
    computed: {
    ...
    methods: {
      show (type, subType, organType, status,vendorid) {
        ...
        var companyid = localStorage.getItem("receiptreq_companyid");
