...
<script>
  import { queryDepartTreeList,queryDepartTreeSync } from '@/api/api'
  export default {
    name: 'JSelectDepartModal',
    props:['modalWidth','multi','rootOpened','departId', 'store', 'text','treeOpera',"pid"],
    data(){
      return {
        visible:false,
    ...
    methods:{
      ...
      loadDepart(){
        ...
        if(this.pid != '' && this.pid != undefined){
          ...
          queryDepartTreeSync(params).then(res=>{
          if(res.success){
            let arr = [...res.result]
