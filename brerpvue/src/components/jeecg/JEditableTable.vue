                ...
                <div class="td" v-for="col in columns" v-show="col.type !== formTypes.hidden" :key="col.key"
                  ...
                  <template v-for="(id, i) in [`${col.key}${row.id}`]">
                    ...
                    <template v-else-if="col.type === formTypes.select">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <a-select v-if="isEditRow(row, col)" :id="id" :key="i" v-bind="buildProps(row, col)"
                          ...
                          @blur="(v) => handleBlurSearch(v, id, row, col)" allowClear />
                        <span v-else class="j-td-span no-edit" :class="{ disabled: buildProps(row, col).disabled }"
                    ...
                    <template v-else-if="col.type === formTypes.sel_material">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <c-select-material v-if="isEditRow(row, col)" :id="id" :key="i" v-bind="buildProps(row, col)"
                          style="width: 100%;" :value="materialCompValues[id]"
                          :placeholder="replaceProps(col, col.placeholder)" :trigger-change="true" :multi="false"
                          @change="(v) => handleChangeMaterialCommon(v, id, row, col)" :buttons="false" />
                        <span v-else class="j-td-span no-edit" :class="{ disabled: buildProps(row, col).disabled }"
                          @click="handleEditRow(row, col)">{{ materialCompValues[id] }}</span>
                      </a-tooltip>
                    </template>

                    <!--供应商选择帮助-->
                    <template v-else-if="col.type === formTypes.sel_vendor">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <c-select-vendor v-if="isEditRow(row, col)" :id="id" :key="i" v-bind="buildProps(row, col)"
                          style="width: 100%;" :value="vendorCompValues[id]"
                          :placeholder="replaceProps(col, col.placeholder)" :trigger-change="true" :multi="false"
                          @change="(v) => handleChangeVendorCommon(v, id, row, col)" :buttons="false" />
                        <span v-else class="j-td-span no-edit" :class="{ disabled: buildProps(row, col).disabled }"
                          @click="handleEditRow(row, col)">{{ vendorCompValues[id] }}</span>
                      </a-tooltip>
                    </template>
                    <!-- date -->
                    ...
                    <template v-else-if="col.type === formTypes.sel_search">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <a-select v-if="isEditRow(row, col)" :id="id" :key="i" 
                        showSearch 
                    ...
                    <div v-else-if="col.type === formTypes.slot" :key="i">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        ...
                        <slot :name="(col.slot || col.slotName) || col.key" :index="rowIndex" :text="slotValues[id]"
                          ...
                          :getValue="() => _getValueForSlot(row.id)" :caseId="caseId"
                          :allValues="_getAllValuesForSlot()" :target="getVM()"
          ...
          <div v-if="showStatisticsRow" class="tr" :style="{
            ...
            <template v-for="col in columns">
              <div :key="col.key" class="td" v-show="col.type !== formTypes.hidden" :style="buildTdStyle(col)">
                <span v-show="col.type === formTypes.inputNumber" style="padding: 0 5px;">{{ statisticsColumns[col.key]
                }}</span>
              </div>
            </template>
...
import { cloneObject, getEventPath, randomNumber, randomString, addZero } from '@/utils/util'
...
import CSelectMaterial from '../../views/erp/base/custom/CSelectMaterial.vue'
import CSelectVendor from '../../views/erp/base/custom/CSelectVendor.vue'
import CSelectCustomer from '../../views/erp/base/custom/CSelectCustomer.vue'
...
export default {
  name: 'JEditableTable',
  components: { JDate, Draggable, JInputPop, JFilePop, CSelectMaterial, CSelectVendor,CSelectCustomer },
  provide () {
    return {
  ...
  methods: {
    ...
    _pushByDataSource (dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
      ...
      let vendorCompValues = { ...this.vendorCompValues }
      ...
      dataSource.forEach((data, newValueIndex) => {
        ...
        this.columns.forEach(column => {
          ...
          } else if (column.type === FormTypes.sel_vendor) {
            vendorCompValues[inputId] = sourceValue
          }
      ...
      this.vendorCompValues = vendorCompValues
      ...
      for (let value of inputValues) {
        ...
        this.columns.forEach(column => {
          ...
          if (column.type === FormTypes.checkbox) {
            ...
            value[column.key] = this.vendorCompValues[inputId]
          }
          else if (column.type === FormTypes.input_pop || column.type === 'textarea') {
    ...
    _getAllValuesForSlot () {
      return cloneObject({
        ...
        vendorCompValues: this.vendorCompValues,
              ...
              if (column.key === newValueKey) {
                ...
                if (!edited) {
                  ...
                  if (column.type === FormTypes.select) {
                    ...
                    edited = this.setOneValue(this.vendorCompValues, modelKey, newValue)
                  }
                  else if (column.type === FormTypes.input_pop || column.type === 'textarea') {
      ...
      newTrEls.forEach(tr => {
        ...
        for (let key in value) {
          if (value.hasOwnProperty(key)) {
            ...
            if (el) {
              if (key === 'itemcode') {
                el.value = addZero(parseInt(idx) + 1)
              } else {
                el.value = value[key]
              }
            }
    ...
    handleChangeVendorCommon (value, id, row, column) {
      this.vendorCompValues = this.bindValuesChange(value, id, 'vendorCompValues')
      this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')
      //触发valuechange事件
      this.elemValueChange(FormTypes.sel_vendor, row, column, value)
    },
    ...
    buildProps (row, col) {
      ...
      if (typeof col.canedit === 'function') {
        let customcanedit = col.canedit;
        props["disabled"] = customcanedit(row, col);
      }
      // 判断select是否允许输入
