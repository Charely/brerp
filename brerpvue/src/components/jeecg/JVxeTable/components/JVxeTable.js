...
export default {
  ...
  props: {
    ...
    // 选择行类型
    rowSelectionType: PropTypes.oneOf(['checkbox', 'radio']).def('radio'),
    // 是否可展开行
  ...
  computed: {
    ...
    vxeColumns() {
      this._innerColumns.forEach(column => {
        ...
        if (column.$type === JVXETypes.datetime 
          || column.$type === JVXETypes.userSelect
           || column.$type === JVXETypes.departSelect
           || column.$type === JVXETypes.materialSelect) {
        ...
        if(column.$type === JVXETypes.popup){
          let width = column.width && column.width.endsWith('px')?Number.parseInt(column.width.substr(0,column.width.length-2)):0;
          if(width <= 200){
            column.width = '200px'
          }
  ...
  methods: {
    ...
    async pushRows(rows = {}, options = {}) {
      ...
      for(let i=0;i<result.rows.length;i++){
        if(result.rows[i].id.indexOf("row_") > -1 ){
            result.rows[i].id = uuid();
        }
      }
      if (setActive) {
        // 激活最后一行的编辑模式
